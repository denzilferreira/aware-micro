  ...
  override fun start(startPromise: Promise<Void>?) {
    ...
    configReader.getConfig { config ->
      if (config.succeeded() && config.result().containsKey("server")) {
        ...
        eventBus.consumer<String>("createTable") { receivedMessage ->
          ...
          sqlClient.getConnection { result ->
            if (result.succeeded()) {
              ...
              connection.query("CREATE TABLE IF NOT EXISTS `${receivedMessage.body()}` (`_id` INT UNSIGNED AUTO_INCREMENT PRIMARY KEY, `timestamp` DOUBLE NOT NULL, `device_id` VARCHAR(128) NOT NULL, `data` TEXT NOT NULL)") {
                if (result.failed()) {
        ...
        eventBus.consumer<JsonObject>("insertTable") { receivedMessage ->
